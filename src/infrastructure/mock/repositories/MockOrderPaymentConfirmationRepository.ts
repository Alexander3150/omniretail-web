import type { InventoryReservation } from "@/core/entities";
  ...
  implements OrderPaymentConfirmationRepository
{
  async confirm(input: ConfirmOrderPaymentInput): Promise<ConfirmOrderPaymentResult> {
    ...
    const result = this.store.transact<ConfirmationMutationResult>((db) => {
      ...
      if (!ecommercePaymentPolicy.isImmediateMockMethod(payment.method)) {
        throw new Error(`Payment method does not allow immediate mock approval: ${payment.method}`);
      }
