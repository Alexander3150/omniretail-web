  ...
  if (!orderNumber) {
    return (
      <main className="mx-auto max-w-2xl px-5 py-10">
        ...
        <p className="mt-4 text-[var(--color-text-muted)]">No hay un pedido reciente para confirmar.</p>
    ...
    <main className="mx-auto max-w-2xl px-5 py-10">
      <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {trackingToken ? (
            <Link className="rounded-md bg-[var(--color-primary)] px-4 py-2 font-semibold text-[var(--color-topbar)]" href={`/pedido/seguimiento/${trackingToken}`}>
              Ver seguimiento
            </Link>
          ) : null}
