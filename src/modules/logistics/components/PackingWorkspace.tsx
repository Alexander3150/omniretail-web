...
function PackingPreparation(props: PackingWorkspaceProps & { detail: PackingDetailDto }) {
  const { detail } = props;
  const homeDelivery = detail.deliveryMethod === DeliveryMethod.home_delivery;
        ...
        <header className="border-b border-[var(--color-border)] bg-[var(--color-app-background)] p-4 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <StatusBadge status={homeDelivery ? "Envío a domicilio" : "Retiro en tienda/bodega"} tone={homeDelivery ? "info" : "warning"} />
    ...
    <>
      <section className="rounded-xl border border-[var(--color-border)] bg-white shadow-sm">
        <header className="border-b border-[var(--color-border)] p-4 sm:p-5">
          ...
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">Vista de preparación para envío a domicilio.</p>
        </header>
        <div className="space-y-4 p-4 sm:p-5">
          <section className="rounded-lg bg-[var(--color-app-background)] p-4">
            <h3 className="font-semibold text-[var(--color-title)]">Datos del destinatario</h3>
            {address ? (
          ...
          {labelReady ? <section className="rounded-lg border-2 border-[var(--color-structure)] p-5">
            ...
            <div className="mt-4 space-y-3 text-sm">
              <div><p className="text-xs text-[var(--color-text-muted)]">Destinatario</p><p className="font-semibold">{address?.recipientName ?? detail.customerName}</p></div>
              <div><p className="text-xs text-[var(--color-text-muted)]">Dirección y teléfono</p><p className="font-semibold">{address ? `${formatAddress(address)} · ${address.recipientPhone ?? "Sin teléfono"}` : "No disponible"}</p></div>
            </div>
      ...
      <ConfirmDialog
        ...
        message="El pedido quedará listo para despacho. La guía y el Dispatch se gestionarán posteriormente."
...
function PackingCompletion({ completion }: { completion: { orderReference: string; orderStatus: OrderStatus } }) {
  ...
  return (
    <section className="flex min-h-[30rem] items-center justify-center rounded-xl border border-[var(--color-border)] bg-white p-6 text-center shadow-sm">
      <div className="max-w-lg">
        ...
        <StatusBadge status={completion.orderStatus} tone="success" />
        <h2 className="mt-4 text-xl font-bold text-[var(--color-title)]">{delivered ? "Entrega confirmada" : "Empaque finalizado"}</h2>
...
function printPackingLabel(detail: PackingDetailDto): boolean {
  ...
  appendText(document, main, "p", "OMNIRETAIL · DESPACHO", "brand");
  appendText(document, main, "h1", detail.orderReference);
  const list = document.createElement("dl");
  appendLabelField(document, list, "Destinatario", address?.recipientName ?? detail.customerName);
