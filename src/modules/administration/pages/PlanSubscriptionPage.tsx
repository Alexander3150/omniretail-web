...
export function PlanSubscriptionPage() {
  ...
  const disablingEcommerce = saved.includes("ecommerce_delivery") && !selected.includes("ecommerce_delivery");
  ...
  async function save() {
    try {
      ...
      setConfirming(false);
      showToast({ title: "No se pudo actualizar", description: caughtError instanceof Error ? caughtError.message : "Inténtelo de nuevo.", tone: "danger" });
    }
  }
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {details && canRead ? <>
      <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm text-[var(--color-text-muted)]">Tu suscripción</p><h2 className="text-xl font-bold text-[var(--color-title)]">MARJYM Base</h2><p className="mt-1 text-sm font-semibold">{money(BASE_MONTHLY_QUETZALES)}/mes</p><p className="mt-1 text-sm text-[var(--color-text-muted)]">Activo desde {formatDate(details.subscription.startedAt)} · Próxima renovación {formatDate(details.nextRenewalAt)}</p></div><StatusBadge status={details.subscription.status} /></div>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(300px,380px)]">
        <section className="space-y-3" aria-labelledby="subscription-catalog-title">
          ...
          {!canManage ? <p className="text-sm text-[var(--color-text-muted)]">Se requiere admin.plans.manage para modificar la suscripción.</p> : null}
        </section>
      ...
      <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm"><h2 className="font-semibold">Historial de facturación</h2><p className="mt-1 text-xs text-[var(--color-text-muted)]">Estos registros no son facturas fiscales ni comprobantes de pago.</p>{details.invoices.length === 0 ? <p className="mt-4 text-sm">Aún no hay ciclos registrados.</p> : <ul className="mt-3 divide-y divide-[var(--color-border)]">{details.invoices.map((invoice) => <li key={invoice.id} className="py-3 text-sm"><details><summary className="flex cursor-pointer flex-wrap justify-between gap-2"><span>Ciclo desde {formatDate(invoice.cycleStart)}</span><strong>{money(invoice.totalQuetzales)}</strong></summary><ul className="mt-3 space-y-1 pl-4 text-[var(--color-text-muted)]"><li className="flex justify-between gap-3"><span>MARJYM Base</span><span>{money(invoice.baseQuetzales)}</span></li>{invoice.addonLines.map((line) => <li key={line.code} className="flex justify-between gap-3"><span>{line.name}</span><span>{money(line.amountQuetzales)}</span></li>)}</ul></details></li>)}</ul>}</section>
    </> : null}
    <ConfirmDialog open={confirming} title="Actualizar suscripción" message={disablingEcommerce ? `Al desactivar E-commerce + Entregas se bloquearán nuevas compras en la tienda online. Los pedidos existentes podrán continuar su preparación, despacho y seguimiento. El total proyectado de ${money(subscriptionTotalQuetzales(selected))}/mes se aplicará en el próximo ciclo, sin cobro inmediato.` : `El acceso a los módulos cambiará de inmediato. El total proyectado de ${money(subscriptionTotalQuetzales(selected))}/mes se aplicará en el próximo ciclo, sin cobro inmediato.`} confirmLabel="Guardar cambios" cancelLabel="Cancelar" onConfirm={() => void save()} onCancel={() => setConfirming(false)} />
  </div>;
}
