    ...
    <div className="min-w-0 space-y-5">
      ...
      <section className="min-w-0 overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-sm">
        ...
        <div className="grid gap-5 p-5 sm:p-7 md:grid-cols-2">
          <section className="min-w-0 rounded-xl border border-[var(--color-border)] p-4"><h2 className="font-black text-[var(--color-text)]">Entrega</h2>{order.deliveryAddress ? <div className="mt-3 break-words text-sm text-[var(--color-text-muted)]"><p className="font-bold text-[var(--color-text)]">{order.deliveryAddress.recipientName}</p><p>{order.deliveryAddress.line1}{order.deliveryAddress.line2 ? `, ${order.deliveryAddress.line2}` : ""}</p><p>{order.deliveryAddress.city}{order.deliveryAddress.department ? `, ${order.deliveryAddress.department}` : ""}</p>{order.deliveryAddress.recipientPhone ? <p className="mt-2">Teléfono: {order.deliveryAddress.recipientPhone}</p> : null}</div> : <p className="mt-3 text-sm text-[var(--color-text-muted)]">Sin entrega a domicilio.</p>}</section>
