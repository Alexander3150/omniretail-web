...
`DispatchRepository.confirm` es el unico boundary de confirmacion y reemplaza el CRUD generico como autoridad. Recibe scope confiable (`tenantId`, `branchId`, actor) desde `DispatchAuthorizationContext`, valida Order/Picking/reservas/incidencias y usa `Order.transportMode`. `third_party` exige carrier y tracking; `own_fleet` permite ambos opcionales. Persiste en una transaccion Dispatch+Order+operacion idempotente y, cuando aplica, una Notification email simulada deduplicada. Sus lecturas son tenant scoped y opcionalmente branch scoped. No modifica ningun contrato de inventario.
